    ...
    window.connectElgatoStreamDeckSocket = (
      ...
      this.$SD.on("keyDown", (message) => {
        if (this.$HA) {
          ...
          if (settings.domain === "weather") {
            this.$HA.getState(settings.entityId, stateDetails);
          }

          if (settings.service) {
            this.$HA.callService(
              settings.service,
              new Entity(settings.entityId)
            );
          }
        }
      });
      ...
      const stateDetails = (event, entityId) => {
        const list = JSON.parse(event.data).result;
        let newId;
        if (entityId.split(".")[0] === "weather") {
          newId = `zone.${entityId.split(".")[1]}`;
        } else newId = entityId;
        let state;
        list.forEach((item) => {
          if (item.entity_id === newId) {
            state = item;
          }
        });
        if (entityId.split(".")[0] === "weather") {
          this.$SD.openUrl(
            `https://forecast.weather.gov/MapClick.php?lon=${state.attributes.longitude}&lat=${state.attributes.latitude}`
          );
        }
      };
      ...
      const updateState = (stateMessage) => {
        ...
        let changedContexts = Object.keys(this.actionSettings).filter(
          (key) => this.actionSettings[key].entityId === entity.entityId
        );
        ...
        changedContexts.forEach((currentContext) => {
          ...
          console.log(
            `Finding image for context ${currentContext}: ${entity.domain}.${deviceClass}(${state})`
          );
          ...
          let svg;
          if (
            IconFactory[entity.domain] &&
            IconFactory[entity.domain][deviceClass]
          ) {
            // console.log(`... sucess!`);
            // domain, class, state
            svg = IconFactory[entity.domain][deviceClass](
              state,
              stateAttributes,
              labelTemplate
            );
            ...
            IconFactory[entity.domain] &&
            IconFactory[entity.domain]["default"]
          ) {
            //console.log(`... sucess (fallback)!`);
            svg = IconFactory[entity.domain]["default"](
              state,
              stateAttributes,
              labelTemplate
            );
          ...
          if (contextSettings.useCustomTitle) {
            const customTitle = new Handlebars.compile(
              contextSettings.buttonTitle
