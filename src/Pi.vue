      ...
      <div class="sdpi-item-group" id="messagegroup_items">
        <div class="sdpi-item">
          <div class="sdpi-item-label">
            <label for="serverUrl">Home Assistant Server URL</label>
          </div>
          <input
            v-model="serverUrl"
      ...
      <div class="sdpi-item-group" id="messagegroup2_items">
        ...
        <div class="sdpi-item">
          ...
          <select class="sdpi-item-value select" id="entity" v-model="entity">
            <option
              v-for="entity in domainEntities"
              v-bind:key="entity.value.entityId"
        ...
        <div class="sdpi-item">
          ...
          <select class="sdpi-item-value select" id="service" v-model="service">
            <option
              ...
              v-bind:value="service.serviceId"
            >
        ...
        <template v-if="useCustomTitle">
          <div class="sdpi-item">
            <div class="sdpi-item-label">
              <label for="txtbuttonTitle">Title Template*</label>
            </div>
            <input
              v-model="buttonTitle"
              class="sdpi-item-value"
        ...
        <template v-if="useCustomButtonLabels">
          <div class="sdpi-item">
            <div class="sdpi-item-label">
              <label for="txtLine1">Button line 1</label>
            </div>
            <input
          ...
          <div class="sdpi-item">
            <div class="sdpi-item-label">
              <label for="txtLine2">Button line 2</label>
            </div>
            <input
          ...
          <div class="sdpi-item">
            <div class="sdpi-item-label">
              <label for="txtLine3">Button line 3</label>
            </div>
            <input
...
export default {
  ...
  computed: {
    ...
    domainServices: function () {
      return Object.keys(this.availableServices[this.domain] || []).map(
        (key) => {
          return {
            serviceId: key,
    ...
    domainEntities: function () {
      return this.availableEntities.filter(
        (entityInfo) => entityInfo.value.domain === this.domain
      );
    },

    entityAttributes: function () {
      let entityAttributes = this.availableAttributes.find(
        (attribute) => attribute.entity.entityId === this.entity
      );
    ...
    connectHomeAssistant: function () {
      ...
      this.$HA = new Homeassistant(this.serverUrl, this.accessToken, () => {
        this.$HA.getStates((states) => {
          ...
          this.availableEntities = states.map((state) => {
            return {
              value: new Entity(state.entity_id),
              text: state.attributes.friendly_name || state.entity_id,
            };
          });
          this.availableEntities = _.orderBy(
            this.availableEntities,
            [(state) => state.text.toLowerCase()],
            ["asc"]
          );
          this.availableAttributes = states.map((state) => {
            return {
